...
public:
    ...
    CDSPResampler(double SrcSampleRate, double DstSampleRate,
                  int MaxInLen, double ReqTransBand = 2.0,
                  double ReqAtten = 206.91,
                  EDSPFilterPhaseResponse ReqPhase = fprLinearPhase,
                  bool UsePower2 = true)
    {
        ...
        else
        {
            ...
            while (CheckSR * SrcSRDiv <= SrcSampleRate)
            {
                ...
                // If downsampling is even deeper, use a less steep filter at this step.
                const double tb = CheckSR * SrcSRDiv <= SrcSampleRate ? 45.0 : 34.0;
     ...
     */
    void oneshot(const int MaxInLen, const double *ip, int iplen, double *op, int oplen)
    {
...
private:
    static const int ConvCountMax = 28; //These convolvers with the built-in 2x up- or downsampling is enough for 1<<(ConvCountMax+1) up- or downsampling.
    CPtrKeeper<CDSPBlockConvolver *> Convs[ConvCountMax]; //Convolvers.
    int ConvCount; //The number of objects defined in the Convs[] array. Equals to 0 if sample rate conversion is not needed.
    CPtrKeeper<CInterpClass *> Interp; //Fractional interpolator object. Equals nullptr if no fractional interpolation is required meaning the "power of 2" resampling is performed or no resampling is performed at all.
    CFixedBuffer<double> ConvBufs[2]; //Intermediate convolution buffers to use, used only when at least 2x upsampling is performed. These buffers are used in flip-flop manner. If nullptr then the input buffer will be used instead.
    CFixedBuffer<double> TmpBuf; //Additional output buffer, can be addressed by the InterpBuf pointer.
    double *InterpBuf; //Final output interpolation buffer to use. If nullptr then the input buffer will be used instead. Otherwise this pointer points to either ConvBufs or TmpBuf.
};
 ...
 * details.
 */
class CDSPResampler16 : public CDSPResampler<CDSPFracInterpolator<18, 137>>
{
public:
    /**
     ...
     */
    CDSPResampler16(double SrcSampleRate, double DstSampleRate,
                    int MaxInLen, double ReqTransBand = 2.0)
 ...
 */
class CDSPResampler16IR : public CDSPResampler<CDSPFracInterpolator<14, 67>>
{
public:
    /**
     ...
     * @param ReqTransBand Required transition band, in percent.
     */
    CDSPResampler16IR(double SrcSampleRate, double DstSampleRate, int MaxInLen, double ReqTransBand = 2.0)
 ...
 */
class CDSPResampler24 : public CDSPResampler<CDSPFracInterpolator<24, 673>>
{
public:
    /**
     ...
     * @param ReqTransBand Required transition band, in percent.
     */
    CDSPResampler24(double SrcSampleRate, double DstSampleRate, int MaxInLen, double ReqTransBand = 2.0)
