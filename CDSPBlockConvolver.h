...
public:
     ...
     * part is available).
     */
    CDSPBlockConvolver(CDSPFIRFilter &aFilter, int aUpFactor,
                       int aDownFactor, double PrevLatency = 0.0,
                       bool aDoConsumeLatency = true) :
        Filter(&aFilter),
        UpFactor(aUpFactor),
    ...
    virtual int getMaxOutLen(const int MaxInLen) const
    {
        ...
        return (MaxInLen * UpFactor + InputDelay + DownFactor - 1) / DownFactor;
    }
    ...
    virtual void clear()
    {
        ...
        else
        {
            ...
            if (DownShift > 0)
            {
                memset(&CurOutput[0], 0, (BlockLen2 >> DownShift) * sizeof(double));
            }
            else
            {
                memset(&CurOutput[BlockLen2 - OutOffset], 0, OutOffset * sizeof(double));
                memset(&CurOutput[0], 0, (InputLen - OutOffset) * sizeof(double));
            }
        }
    ...
    virtual int process(double *ip, int l0, double *&op0)
    {
        ...
        while (l > 0)
        {
            ...
            if (l < InDataLeft)
            {
                ...
                if (UpShift >= 0)
                    memcpy(&CurInput[Offs >> UpShift], ip, (l >> UpShift) * sizeof(double));
                else
            ...
            if (UpShift >= 0)
            {
                ...
                memcpy(&CurInput[Offs >> UpShift], ip, bu * sizeof(double));
            ...
            if (Filter->isZeroPhase())
                (*fftout)->multiplyBlocksZ(Filter->getKernelBlock(), CurInput);
            else
            ...
            if (DownShift > 0)
            {
                ...
                CurInput[1] = Filter->getKernelBlock()[z] * CurInput[z];
            }
...
private:
    ...
    CPtrKeeper<CDSPRealFFTKeeper *> fftin; //FFT object 1, used to produce the input spectrum (can embed the "power of 2" upsampling).
    CPtrKeeper<CDSPRealFFTKeeper *> ffto2; //FFT object 2 (can be nullptr).
    CDSPRealFFTKeeper *fftout; //FFT object used to produce the output signal (can embed the "power of 2" downsampling), may point to either "fftin" or "ffto2".
    int UpFactor; //Upsampling factor.
    int DownFactor; //Downsampling factor.
    bool DoConsumeLatency; //"True" if the output latency should be consumed. Does not apply to the fractional part of the latency (if such part is available).
    int BlockLen2; //Equals block length * 2.
    int OutOffset; //Output offset, depends on filter's introduced latency.
    int PrevInputLen; //The length of previous input data saved, used for /overlap.
    int InputLen; //The number of input samples that should be accumulated before the input block is processed.
    int Latency; //Processing latency, in samples.
    double LatencyFrac; //Fractional latency, in samples, that is left in the output signal.
    int UpShift; //"Power of 2" upsampling shift. Equals -1 if UpFactor is not a "power of 2" value. Equals 0 if UpFactor equals 1.
    int DownShift; //"Power of 2" downsampling shift. Equals -1 if DownFactor is not a "power of 2". Equals 0 if DownFactor equals 1.
    int InputDelay; //Additional input delay, in samples. Used to make the output latency divisible by DownShift. Used only if UpShift <= 0 and DownShift > 0.
    CFixedBuffer<double> WorkBlocks; //Previous input data, input and output data blocks, overall capacity = BlockLen2 * 2 + PrevInputLen. Used in the flip-flop manner.
    ...
    int InDataLeft; //Samples left before processing input and output FFT blocks. Initialized to InputLen on clear.
    int LatencyLeft; //Latency in samples left to skip.
    int UpSkip; //The current upsampling sample skip (value in the range 0 to UpFactor - 1).
    int UpSkipInit; //The initial UpSkip value after clear().
    int DownSkip; //The current downsampling sample skip (value in the range 0 to DownFactor - 1). Not used if DownShift > 0.
    ...
    void copyToOutput(int Offs, double *&op0, int b, int &l0)
    {
        ...
        if (DownShift > 0)
        {
            ...
            {
                b = (b + df - 1) >> DownShift;
                memcpy(op0, &CurOutput[Offs >> DownShift], b * sizeof(double));
