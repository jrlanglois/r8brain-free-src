...
public:
     ...
     *
     * @param Params Window function's parameters. If nullptr then the built-in
    ...
    void calculate(const double *const Params = nullptr)
    {
        ...
        if (FilterLen <= 20)
        {
            for (int i = -pc2 + 1; i <= FilterFracs + pc2; i++)
            {
        ...
        if (InterpPoints == 8)
        {
            if (ElementSize == 3)
            {
                // Calculate 2nd order spline (polynomial) interpolation coefficients using 8 points.
                while (p < TableEnd)
                {
            ...
            {
                // Calculate 3rd order spline (polynomial) interpolation coefficients using 8 points.
                while (p < TableEnd)
                {
...
private:
    static const int FilterSize = FilterLen * ElementSize; //This constant specifies the "size" of a single filter in "double" elements.
    double Table[FilterSize * (FilterFracs + InterpPoints)]; //The table of fractional delay filters for all discrete fractional x = 0..1 sample positions, and interpolation coefficients.
};
...
public:
    ...
    CDSPFracInterpolator(double aSrcSampleRate, double aDstSampleRate, double aInitFracPos) :
    ...
    virtual void clear()
    {
        BufLeft = 0;
        WritePos = 0;
        ReadPos = BufLen - FilterLenD2Minus1; // Set "read" position to account for filter's latency at zero fractional delay which equals to FilterLenD2Minus1.
    ...
    virtual int process(double *ip, int l, double *&op0)
    {
        ...
        while (l > 0)
        {
            ...
            const int b = min(min(l, BufLen - WritePos),  BufLeftMax - BufLeft);
            ...
            while (BufLeft >= FilterLenD2Plus1)
            {
                ...
                x -= fti;               // Coefficient for interpolation between adjacent fractional delay filters.
                const double x2 = x * x;
                ...
                {
#if ! R8B_FLTTEST
                    s += (ftp[ii] + ftp[ii + 1] * x + ftp[ii + 2] * x2) * rp[i];
#else
                    s += (ftp[ii] + ftp[ii + 1] * x + ftp[ii + 2] * x2 + ftp[ii + 3] * x3) * rp[i];
#endif
                ...
                *op = s;
                ...
                InCounter++;
                const double NextInPos = (InCounter + InPosShift) * SrcSampleRate / DstSampleRate;
        ...
        if (InCounter > 1000)
        {
            // Reset the interpolation position counter to achieve a higher sample timing precision.
            InCounter = 0;
            InPosInt = 0;
...
private:
#if ! R8B_FLTTEST
    static const int FilterElementSize = 3; //The number of "doubles" a single filter tap consists of (includes interpolation coefficients).
#else
    static const int FilterElementSize = 4; //The number of "doubles" a single filter tap consists of (includes interpolation /coefficients). During filter testing a higher precision interpolation is used.
#endif

    static const int FilterLenD2 = FilterLen >> 1; //= FilterLen / 2.
    static const int FilterLenD2Minus1 = FilterLenD2 - 1; //= /FilterLen / 2 - 1. This value also equals to filter's latency in samples (taps).
    static const int FilterLenD2Plus1 = FilterLenD2 + 1; //= FilterLen / 2 + 1.
    static const int BufLenBits = 8; //The length of the ring buffer, expressed as Nth power of 2. This value can be reduced if it is
        //known that only short input buffers will be passed to the interpolator. The minimum value of this parameter is 5, and
        //1 << BufLenBits should be at least 3 times larger than the FilterLen.
    static const int BufLen = 1 << BufLenBits; //The length of the ring buffer. The actual length is twice as long to allow "beyond max position" positioning.
    static const int BufLenMask = BufLen - 1; //Mask used for quick buffer position wrapping.
    static const int BufLeftMax = BufLen - FilterLenD2Minus1; //The number of new samples that the ring buffer can hold at most. The remaining FilterLenD2Minus1 samples hold "previous" input samples for the filter.
    double Buf[BufLen * 2]; //The ring buffer.
    double SrcSampleRate; //Source sample rate.
    double DstSampleRate; //Destination sample rate.
    double InitFracPos; //Initial fractional position, in samples, in the range [0; 1).
    int BufLeft; //The number of samples left in the buffer to process. When this value is below FilterLenD2Plus1, the interpolation cycle ends.
    int WritePos; //The current buffer write position. Incremented together with the BufLeft variable.
...
#if ! R8B_FLTTEST
    static const CDSPFracDelayFilterBank<FilterLen, FilterFracs, 3, 8>
        FilterBank; //Filter bank object, defined statically if no filter test takes place.
#else
public:
    CDSPFracDelayFilterBank<FilterLen, FilterFracs, 4, 8> FilterBank; //Filter bank object, defined as a member variable to allow for recalculation.
#endif
};

#if ! R8B_FLTTEST
template <int FilterLen, int FilterFracs>
const CDSPFracDelayFilterBank<FilterLen, FilterFracs, 3, 8> CDSPFracInterpolator<FilterLen, FilterFracs>::FilterBank;
#endif
