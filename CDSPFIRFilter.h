...
private:
    ...
    int BlockLenBits; //Block length used to store *this FIR filter,
        //expressed as Nth power of 2. This value is used directly by the convolver.
    ...
    void buildLPFilter(const double *const ExtAttenCorrs)
    {
        ...
        pwr = 7.43932822146293e-8 * sqr(atten) + 0.000102747434588003 * cos(0.00785021930010397 * atten) * cos(0.633854318781239 + 0.103208573657699 * atten) - 0.00798132247867036 -
              0.000903555213543865 * atten - 0.0969365532127236 * exp(0.0779275237937911 * atten) - 1.37304948662012e-5 * atten * cos(0.00785021930010397 * atten);
        ...
        if (pwr <= 0.067665322581)
        {
            if (tb >= 0.25)
            {
                ...
                fo1 = 0.987205355829873 * tb + 1.00011788929851 * atan2(
                                                                      ...
                                                                      hl + -1.14861472207245 / (hl - 14.1821147585957) + pow(0.9521145021664, pow(atan2(1.12018764830637, tb), 2.10988901686912 * hl - 20.9691278378345)));
            }
            else if (tb >= 0.10)
            {
                hl = (1.56688617018066 + 142.064321294568 * pwr +
                      0.00419441117131136 * cos(243.633511747297 * pwr) -
                      0.022953443903576 * atten - 0.026629568860284 * cos(127.715550622571 * pwr)) /
                     tb;
            ...
            else
            {
                ...
                fo1 = 2.291956939 * tb + 0.01942450693 * sqr(tb) * hl -
                      4.67538973161837 * pwr * tb - 1.668433124 * tb * pow(pwr, pwr);
            }
        }
        else
        {
            if (tb >= 0.25)
            {
                hl = (1.50258368698213 + 158.556968859477 * asinh(pwr) * tanh(57.9466246871383 * tanh(pwr)) -
                      0.0105440479814834 * atten) /
                     tb;

                fo1 = 0.994024401639321 * tb + (-0.236282717577215 -
                                                6.8724924545387 * sqrt(sin(pwr))) /
                                                   hl;
            }
            else if (tb >= 0.10)
            {
                hl = (1.50277377248945 + 158.222625721046 * asinh(pwr) * tanh(1.02875299001715 + 42.072277322604 * pwr) -
                      0.0108380943845632 * atten) /
                     tb;

                fo1 = 0.992539376734551 * tb + (-0.251747813037178 -
                                                6.74159892452584 * sqrt(tanh(tanh(tan(pwr))))) /
                                                   hl;
            }
            else
            {
        ...
        if (IsZeroPhase)
        {
            ...
            // Time-shift the filter so that zero-phase response is produced. Simultaneously multiply by "s".
            for (int i = 0; i <= sinc.fl2; i++)
                KernelBlock[i] = KernelBlock[sinc.fl2 + i] * s;

            for (int i = 1; i <= sinc.fl2; i++)
...
public:
     ...
     */
    static CDSPFIRFilter &getLPFilter(double ReqNormFreq,  double ReqTransBand, double ReqAtten,
                                      EDSPFilterPhaseResponse ReqPhase, double ReqGain,
                                      const double *const AttenCorrs = nullptr)
    {
        ...
        {
            // Create a new filter object (with RefCount == 1) and build the filter kernel.
