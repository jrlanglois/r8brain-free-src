...
public:
    ...
    union
    {
        struct
        {
            ...
            double Freq2; //Required corner circular frequency 2 [0; pi]. Used only in the generateBand() function. The range
                          //[Freq1; Freq2] defines a pass band for the generateBand() function.
        };
    ...
    enum EWindowFunctionType
    {
        ...
        wftVaneev //Vaneev window function, mainly used for short fractional delay filters, requires 4 cosine width parameters, plus the "Power" parameter which is mandatory.
    };
     ...
     * @param Params Window function's parameters. If nullptr, the table values
     * may be used.
     ...
     * last value in the Params array. If Params is nullptr, the table or default
     * value of -1.0 (off) will be used.
     */
    void initWindow(const EWindowFunctionType WinType = wftCosine,
                    const double *const Params = nullptr, bool UsePower = false)
    {
        R8BASSERT(Len2 >= 2.0);
     ...
     * value of -1.0 (off) will be used.
     */
    void initBand(EWindowFunctionType WinType = wftCosine,
                  const double *const Params = nullptr, bool UsePower = false)
    {
        R8BASSERT(Len2 >= 2.0);
     ...
     * value of -1.0 (off) will be used.
     */
    void initHilbert(EWindowFunctionType WinType = wftCosine,
                     const double *const Params = nullptr, bool UsePower = false)
    {
        R8BASSERT(Len2 >= 2.0);
     ...
     * value of -1.0 (off) will be used.
     */
    void initFrac(EWindowFunctionType WinType = wftCosine,
                  const double *const Params = nullptr, bool UsePower = false)
    {
        R8BASSERT(Len2 >= 2.0);
    ...
    {
        return 0.355768 + 0.487396 * w1.generate() + 0.144232 * w2.generate() + 0.012604 * w3.generate();
    }
    ...
    {
        return 0.3635819 + 0.4891775 * w1.generate() + 0.1365995 * w2.generate() + 0.0106411 * w3.generate();
    }
    ...
    {
        const double f = exp(-0.5 * sqr(wn / GaussianSigma + GaussianSigmaFrac));
        wn++;
        return f;
    }
    ...
    void generateBand(T *op, CWindowFunc wfunc = &CDSPSincFilterGen::calcWindowBlackman)
    {
        ...
        if (Power < 0.0)
        {
            *op = (Freq2 - Freq1) * (*this.*wfunc)() / M_PI;
            ...
            while (t <= fl2)
            {
                const double v = (f2.generate() - f1.generate()) * (*this.*wfunc)() / t / M_PI;
        ...
        else
        {
            *op = (Freq2 - Freq1) * pows((*this.*wfunc)(), Power) / M_PI;
            ...
            while (t <= fl2)
            {
                const double v = (f2.generate() - f1.generate()) * pows((*this.*wfunc)(), Power) / t / M_PI;
    ...
    void generateHilbert(T *op, CWindowFunc wfunc = &CDSPSincFilterGen::calcWindowBlackman)
    {
        ...
        *op = 0.0;
        ...
        else
        {
            while (t <= fl2)
            {
                const double v = fvalues[t & 1] * pows((*this.*wfunc)(), Power) / t / M_PI;
     ...
     */
    template <class T>
    void generateFrac(T *op, CWindowFunc wfunc = &CDSPSincFilterGen::calcWindowBlackman, int opinc = 1)
    {
        R8BASSERT(opinc != 0);
        ...
        if (Power < 0.0)
        {
            ...
            *op = (abs(ut) <= 1e-13 ? (*this.*wfunc)() : f[t & 1] * (*this.*wfunc)() / ut / M_PI);
        ...
        else
        {
            while (t < mt)
            {
                *op = f[t & 1] * pows((*this.*wfunc)(), Power) / (t + FracDelay) / M_PI;
            ...
            double ut = t + FracDelay;
            *op = (abs(ut) <= 1e-13 ? pows((*this.*wfunc)(), Power) : f[t & 1] * pows((*this.*wfunc)(), Power) / ut / M_PI);
...
private:
    ...
    int wn; //Window function integer position. 0 - center of the window function. This variable may not be used by some window functions.
     ...
     *
     * @param Params Function parameters. If nullptr, the table values will be
     * used. If not nullptr, the first parameter should specify the "Beta" value.
     * @param UsePower "True" if the power factor should be used to raise the
     * window function.
     ...
     */
    void setWindowKaiser(const double *Params, bool UsePower, bool IsCentered)
    {
        wn = (IsCentered ? 0 : -fl2);
        ...
        else
        {
            ...
            Power = (UsePower ? abs(Params[1]) : -1.0);
        }
     ...
     *
     * @param Params Function parameters. If nullptr, the table values will be
     * used. If not nullptr, the first parameter should specify the "Sigma"
     * value.
     * @param UsePower "True" if the power factor should be used to raise the
     * window function.
     * @param IsCentered "True" if centered window should be used. This
    ...
    void setWindowGaussian(const double *Params, bool UsePower, bool IsCentered)
    {
        wn = IsCentered ? 0 : -fl2;
        ...
        {
            GaussianSigma = clampr(abs(Params[0]), 1e-1, 100.0);
            Power = (UsePower ? abs(Params[1]) : -1.0);
        }
     ...
     *
     * @param Params Function parameters. If nullptr, the table values will be
     * used. If not nullptr, the first 4 parameters should specify the cosine
     * multipliers while the fifth parameter should specify the "Power" value.
     * @param IsCentered "True" if centered window should be used. This
     * parameter usually equals to "false" for fractional delay filters only.
     */
    void setWindowVaneev(const double *Params, bool IsCentered)
    {
     ...
     */
    void setWindow(EWindowFunctionType WinType, const double *const Params, bool UsePower, bool IsCentered, double UseFracDelay = 0.0)
    {
