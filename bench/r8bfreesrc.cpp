...
VOXMAIN
{
	...
	const double d = (OutSampleRate > inf.SampleRate ? OutSampleRate / inf.SampleRate : inf.SampleRate / OutSampleRate);
	...
	for (int i = 0; i < inf.ChannelCount; i++)
	{
		...
		Resamps[i] = new r8b::CDSPResampler24(inf.SampleRate, OutSampleRate, InBufCapacity, tb);
	}

	int64_t ol = (int64_t)(inf.SampleCount * OutSampleRate / inf.SampleRate);
	...
	while (ol > 0)
	{
		...
		int WriteCount; // At initial steps this variable can be equal to 0 after resampler. Same number for all channels.
