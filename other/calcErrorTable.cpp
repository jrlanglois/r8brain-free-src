...
int main()
{
	...
	{
		const double atten = MinAtten + (MaxAtten - MinAtten) * i / (AttenSteps - 1);
		double tb;

		for (tb = 25.0; tb <= 45.0; tb *= 1.003)
		{
			CDSPFIRFilter &f = CDSPFIRFilterCache::getLPFilter(0.5, tb, atten, fprLinearPhase, 1.0);
			...
			calcFIRFilterResponse(&Kernel[0], l, M_PI * (1.0 - tb * 0.01) * 0.5, re, im);
