...
int main()
{
	...
	for (int i = 0; i <= AttenTableSize; i++)
		AttenCorrs[i] = 0.0;
	...
	for (int j = 0; j < IterCount; j++)
	{
		...
		for (int i = 0; i <= AttenSteps; i++)
		{
			...
			{
				CDSPFIRFilter &f = CDSPFIRFilterCache::getLPFilter(0.5, tb, ReqAtten, fprLinearPhase, AttenCorrs);
				...
				memcpy(&Kernel[0], f.getKernelBlock(), (2 * sizeof(double)) << f.getBlockLenBits());
				...
				const int z = (int)floor((ReqAtten - MinAtten) * AttenTableSize / AttenDiff + 0.5);
